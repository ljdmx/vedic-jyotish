...
export function buildPrashnaLocation(draft: PrashnaLocationDraft): { payload: PrashnaLocationPayload | null; error: string | null } {
  ...
  const timezoneText = draft.timezoneOffset.trim();
  const timezoneOffset = Number(timezoneText);
  ...
  if (!timezoneText || !Number.isFinite(timezoneOffset) || timezoneOffset < -720 || timezoneOffset > 840) return { payload: null, error: "请填写 -720 至 840 分钟之间的有效时区" };
