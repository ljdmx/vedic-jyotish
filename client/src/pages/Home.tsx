...
const initialPrashnaLocation = { place: "", latitude: "", longitude: "", timezoneOffset: "" };
...
export default function Home() {
  ...
  const [keepRawReport, setKeepRawReport] = useState(false);
  ...
  const startReportStream = (payload: Record<string, unknown>) => {
    ...
    streamAbortRef.current = streamReport(payload, {
      ...
      onRestart: () => {
        if (!streamRunGuardRef.current.isCurrent(run)) return;
        // 兼容旧服务端的 restart 事件：保留已显示内容，防止生成过程视觉回退到开头。
      ...
      onDone: data => {
        if (!streamRunGuardRef.current.isCurrent(run)) return;
        pendingReportRef.current = data.report.resultMarkdown;
        flushPendingReport();
  ...
  const stopGeneration = () => {
    ...
    setStreamWaitingLong(false);
    reportFlushSchedulerRef.current?.cancel();
    // 仍保持 streamingRef 为 true，先把尚未 flush 的尾部写入既有 DOM。
    flushPendingReport();
    streamingRef.current = false;
    setReportText(pendingReportRef.current);
    setKeepRawReport(Boolean(pendingReportRef.current));
...
function PrashnaPanel({ busy, location, setLocation, resolvePlace, question, setQuestion, run }: { busy: boolean; location: typeof initialPrashnaLocation; setLocation: (v: typeof initialPrashnaLocation) => void; resolvePlace: () => void; question: string; setQuestion: (s: string) => void; run: () => void }) {
  return <IntroLayout>
    <div className="empty-inline mb-4"><strong className="serif text-[#292b2a]">独立问盘裁决</strong><br />起盘时刻由系统当前时间生成，且不读取本命盘、合盘、Tajika 或 KP 内容。系统会先按上升主、问题宫主、月亮、整宫与 Graha Drishti 给出“可推进／条件性推进／建议暂缓”之一，并列出支持、制约与月亮短期观察窗；模型只负责补充解释，不会覆盖该裁决。</div>
    <div className="form-span"><label className="label">本次起盘地点</label><div className="place-field"><input className="field" value={location.place} onChange={e => setLocation({ ...location, place: e.target.value })} placeholder="例如：昆明市；边界地区可填写区县" /><button type="button" className="line-button text-xs shrink-0" onClick={resolvePlace}><MapPin size={13} /> 定位</button></div><p className="mt-2 text-xs text-[#77746d]">城市级地点通常足够；跨城、国界／时区边界或上升接近星座交界时，建议补至区县或街道。无需填写门牌号。</p></div>
    <div className="two-col mt-4"><div><label className="label">纬度（北正南负）</label><input className="field" type="number" onWheel={blurNumberInputOnWheel} step="0.0001" value={location.latitude} onChange={e => setLocation({ ...location, latitude: e.target.value })} placeholder="31.2304" /></div><div><label className="label">经度（东正西负）</label><input className="field" type="number" onWheel={blurNumberInputOnWheel} step="0.0001" value={location.longitude} onChange={e => setLocation({ ...location, longitude: e.target.value })} placeholder="121.4737" /></div><div><label className="label">时区（UTC 偏移）</label><div className="place-field"><input className="field bg-[#f1efe8] text-[#77746d]" type="text" value={location.timezoneOffset === "" ? "定位后自动回填" : `${location.timezoneOffset} 分钟`} readOnly aria-readonly="true" /><span className="shrink-0 text-right text-xs text-[#65796b]">{location.timezoneOffset === "" ? "尚未确定" : formatUtcOffset(location.timezoneOffset)}<br />东正西负 · 分钟</span></div><p className="mt-2 text-xs text-[#77746d]">完成地点定位后，系统会以当前起盘时刻、经纬度与历史时区规则自动回填；该数值仅供复核，不能手动修改。</p></div><div className="empty-inline"><strong className="serif text-[#292b2a]">地点精度</strong><br />经纬度直接参与上升点与整宫计算；自动定位会回填 4 位小数。若上升点不靠近星座交界，城市中心与区县坐标通常不会改变宫位结构。</div></div>
    <label className="label mt-5">你要问的问题</label><textarea className="field min-h-28 resize-y" value={question} onChange={e => setQuestion(e.target.value)} placeholder="例如：我接受这份 offer 后能否顺利入职？" />
    <ActionRow busy={busy} run={run} icon={<Compass size={16} />} label="生成问盘裁决" note="问题须有可观察结果；报告首先给出可复算裁决与证据链，再说明条件和现实验证步骤。" disabled={!question.trim()} />
  </IntroLayout>
}
